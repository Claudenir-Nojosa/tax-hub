...
## Textos do PDF redigidos por IA (task #90)

Botão **"Preencher com IA"** no `ExportarPdfDialog`: a rota `POST /api/reforma-tributaria/textos-pdf` (gpt-4o, persona "pós-doutor em reforma tributária/pós-graduado em direito tributário/especialista consultivo") redige as duas seções em HTML limitado ao subconjunto do editor (div/b/i/u/ul/li/br). Regras do prompt: **Legislações SEMPRE abre com o art. 128 do ADCT** (redução gradual de ICMS/ISS: 9/10 em 2029 → 6/10 em 2032, extinção em 2033, § 1º sobre benefícios fiscais — vale pra toda empresa, pedido explícito do usuário), depois art. 47 (não cumulatividade, § 9º Simples, § 10 não estorno) e, com redução 60% ativa, o art. 133 (ressalva do art. 146). Considerações finais = "o porquê dos valores": antes/depois pro setor + números REAIS do estudo, enviados pelo dialog (total 2026/2033, impacto %, % de fornecedores Regime Regular, créditos 2033 — calculados na hora via `obterDados()` + `calcularQuadroComparativo`; sem dados locais, a IA redige sem os números). O usuário revisa/edita no editor rico antes de salvar e gerar — a IA só preenche o rascunho.

Validação: prompt REAL extraído do arquivo da rota (eval do template) e enviado ao gpt-4o com contexto Art Farma — JSON válido, art. 128/133/47 presentes, números do estudo usados, nenhuma tag fora da lista; rota no dev server responde 401 sem sessão.

## Textos justificados + seta de voltar no wizard (task #91)

- `escreverRico()` (reforma-pdf.ts) reescrita: monta as linhas de cada parágrafo primeiro (greedy, medindo largura real por palavra/estilo) e só então desenha, distribuindo a sobra de espaço igualmente entre as palavras de cada linha — exceto a última linha do parágrafo, que fica alinhada à esquerda (justificação clássica). Validado no PDF real: linhas das seções Legislações/Considerações esticam até a margem direita.
- Header do wizard (`[empresaId]/page.tsx`): seta "←" (ArrowLeft) antes do ícone da balança, link para `/dashboard/reforma-tributaria` — permite voltar pra listagem de qualquer aba do projeto sem usar o botão do navegador.
