    ...
    <div className="max-w-5xl mx-auto py-8 px-4">
      <div className="flex items-start justify-between mb-8">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-bold text-white">Meus Concursos</h1>
          </div>
          <p className="text-sm text-gray-400">Gerencie seus concursos e acesse o módulo de estudos de cada um.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 border-2 border-dashed border-gray-700 rounded-xl">
          ...
          <p className="text-gray-400 mb-4">Nenhum concurso ainda. Crie o primeiro!</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {concursos.map(c => (
            <div key={c.id} className={`rounded-xl border p-5 bg-gray-900 flex flex-col gap-3 transition-all ${c.isPrincipal ? "border-blue-500 shadow-blue-500/20 shadow-lg" : "border-gray-700 hover:border-gray-500"}`}>
              {/* Header */}
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1 flex-wrap">
                    <h3 className="font-semibold text-white text-sm leading-tight">{c.nome}</h3>
                    {c.isPrincipal && <Badge className="text-xs bg-blue-600/20 text-blue-300 border-blue-600/40">Principal</Badge>}
                  </div>
                  {c.orgao && <p className="text-xs text-gray-400 mt-0.5">{c.orgao}</p>}
                </div>
                <div className="flex gap-1 shrink-0">
                  <button onClick={() => { setEditando(c); setModalAberto(true) }} className="text-gray-500 hover:text-blue-400 p-1"><Pencil className="h-3.5 w-3.5" /></button>
                  <button onClick={() => handleExcluir(c.id)} className="text-gray-500 hover:text-red-400 p-1"><Trash2 className="h-3.5 w-3.5" /></button>
                </div>
              </div>
