...
// GET ?projetoId= — lista DCTF do projeto, ordenadas por competência
export async function GET(req: NextRequest) {
  ...
  const declaracoes = await db.declaracaoDctf.findMany({
    where: { projetoId },
...
// DELETE ?id= — remove uma DCTF específica
export async function DELETE(req: NextRequest) {
  ...
  const declaracao = await db.declaracaoDctf.findFirst({
    where: { id },
  ...
  await db.declaracaoDctf.delete({ where: { id } })
