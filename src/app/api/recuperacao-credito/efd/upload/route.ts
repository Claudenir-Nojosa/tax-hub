...
// POST — upload de 1+ EFD ICMS/IPI (.txt), um arquivo = uma competência (mês).
...
export async function POST(req: NextRequest) {
  ...
  const salvos: { competencia: string; arquivoNome: string }[] = []
  ...
  for (const file of files) {
    ...
    try {
      ...
      salvos.push({ competencia: dados.competencia, arquivoNome: file.name })
    } catch (e) {
