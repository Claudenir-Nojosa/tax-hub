...
// GET ?projetoId= — lista declarações EFD ICMS/IPI do projeto, ordenadas por competência
export async function GET(req: NextRequest) {
  ...
  const declaracoes = await db.declaracaoEfdIcmsIpi.findMany({
    where: { projetoId },
...
export async function DELETE(req: NextRequest) {
  ...
  const declaracao = await db.declaracaoEfdIcmsIpi.findFirst({
    where: { id },
  ...
  await db.declaracaoEfdIcmsIpi.delete({ where: { id } })
