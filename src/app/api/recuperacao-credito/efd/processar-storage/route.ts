...
// direto pro Storage, ver .../upload/urls/route.ts). Baixa cada um, delega o
// parsing/persistência pra processarArquivoEfdRecuperacaoCredito (mesma função do upload direto
// via FormData) e apaga o objeto do Storage ao final (sucesso ou erro — é rascunho, a verdade
// fica no Postgres). Processa por ORÇAMENTO DE TEMPO — ver mesmo comentário em
// .../pdf/processar-storage/route.ts.
export async function POST(req: NextRequest) {
  ...
  const salvos: { competencia: string; arquivoNome: string; tipo: string }[] = []
  ...
  for (; i < arquivos.length; i++) {
    ...
    try {
      ...
      const file = new File([blob], nomeOriginal, { type: "text/plain" })
      const resultado = await processarArquivoEfdRecuperacaoCredito(file, projeto, cliente)
      if (resultado.ok) {
        salvos.push({ competencia: resultado.competencia, arquivoNome: nomeOriginal, tipo: resultado.tipo })
      } else {
