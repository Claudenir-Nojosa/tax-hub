...
// GET ?projetoId= — lista ECFs (IRPJ/CSLL) do projeto, ordenadas por ano-calendário
export async function GET(req: NextRequest) {
  ...
  const declaracoes = await db.declaracaoEcf.findMany({
    where: { projetoId },
...
// DELETE ?id= — remove uma ECF específica (ex.: reenviar um ano errado)
export async function DELETE(req: NextRequest) {
  ...
  const declaracao = await db.declaracaoEcf.findFirst({
    where: { id },
  ...
  await db.declaracaoEcf.delete({ where: { id } })
