...
// GET ?projetoId= — lista relatórios de Fontes Pagadoras do projeto, ordenados por ano
export async function GET(req: NextRequest) {
  ...
  const declaracoes = await db.declaracaoFontesPagadoras.findMany({
    where: { projetoId },
...
// DELETE ?id= — remove um relatório específico
export async function DELETE(req: NextRequest) {
  ...
  const declaracao = await db.declaracaoFontesPagadoras.findFirst({
    where: { id },
  ...
  await db.declaracaoFontesPagadoras.delete({ where: { id } })
