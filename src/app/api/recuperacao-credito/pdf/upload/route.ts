...
async function salvarCadastro(
  ...
  const existente = await db.cadastroEmpresa.findUnique({
    where: { projetoId },
  });
  const dados = {
    ...((existente?.dados as object | null) ?? {}),
    ...parcial,
...
export async function POST(req: NextRequest) {
  ...
  for (const file of files) {
    if (!file.name.toLowerCase().endsWith(".pdf")) {
      erros.push({
        arquivo: file.name,
        motivo: "Apenas arquivos PDF são aceitos",
      });
      continue;
    }
    ...
    try {
      ...
      if (detectarFontesPagadoras(textoBruto)) {
        ...
        if (!dados) {
          erros.push({
            arquivo: file.name,
            motivo: "Não foi possível ler o relatório de Fontes Pagadoras",
          });
          continue;
        }
        if (
          somenteDigitos(dados.cnpjBeneficiario) !==
          somenteDigitos(cliente.cnpj)
        ) {
          erros.push({
            arquivo: file.name,
        ...
        if (!dados) {
          erros.push({
            arquivo: file.name,
            motivo: "Não foi possível ler o cabeçalho/débitos da DCTFWeb",
          });
          continue;
        }
        ...
        if (darfs.length === 0) {
          erros.push({
            arquivo: file.name,
            motivo: "Não foi possível extrair nenhum DARF do comprovante",
          });
          continue;
        }
        ...
        const novos = validos.filter(
          (d) => !setExistentes.has(d.numeroDocumento),
        );
        const repetidos = validos.filter((d) =>
          setExistentes.has(d.numeroDocumento),
        );
        ...
        if (!dados) {
          erros.push({
            arquivo: file.name,
            motivo:
              "Não foi possível ler o Comprovante de Inscrição (Consulta CNPJ)",
          });
          continue;
        }
        if (somenteDigitos(dados.cnpj) !== somenteDigitos(cliente.cnpj)) {
        ...
        salvos.push({
          arquivoNome: file.name,
          tipo: "CADASTRO",
          detalhe: `Consulta CNPJ (${dados.nomeEmpresarial})`,
        });
        ...
        if (!dados) {
          erros.push({
            arquivo: file.name,
            motivo: "Não foi possível ler o QSA (Dados Cadastrais)",
          });
          continue;
        }
        if (somenteDigitos(dados.cnpj) !== somenteDigitos(cliente.cnpj)) {
        ...
        salvos.push({
          arquivoNome: file.name,
          tipo: "CADASTRO",
          detalhe: `QSA (${dados.socios.length} sócio(s))`,
        });
      } else if (
        textoBruto.trim().length < 50 ||
        detectarConsultaOptantes(textoBruto)
      ) {
        ...
        const resultado = await extrairSimplesNacionalViaIA(
          uint8ParaIA,
          file.name,
        );
        if (!resultado.ok) {
        ...
        await salvarCadastro(projeto.id, cliente.cnpj, {
          simplesNacional: resultado.dados,
        });
        salvos.push({
          arquivoNome: file.name,
          tipo: "CADASTRO",
          detalhe: `Simples Nacional (${resultado.dados.situacao})`,
        });
      } else {
        ...
        salvos.push({
          arquivoNome: file.name,
          tipo: "PGDAS",
          detalhe: `${dados.competencia} (${dados.tipoDocumento})`,
        });
      }
    } catch (e) {
      erros.push({
