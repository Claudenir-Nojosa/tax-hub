...
// GET ?projetoId= — lista ECDs do projeto, ordenadas por ano
export async function GET(req: NextRequest) {
  ...
  const declaracoes = await db.declaracaoEcd.findMany({
    where: { projetoId },
...
// DELETE ?id= — remove uma ECD específica
export async function DELETE(req: NextRequest) {
  ...
  const declaracao = await db.declaracaoEcd.findFirst({
    where: { id },
  ...
  await db.declaracaoEcd.delete({ where: { id } })
