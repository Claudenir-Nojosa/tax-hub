...
const HEADERS = [
  ...
  "CFOP", "Vlr Base Cálculo ICMS", "Alíquota ICMS", "Vlr ICMS", "Tipo Crédito",
] as const
...
export function montarAbaEntradasEfd(
  ...
  TODOS_HEADERS.forEach((nome, i) => celula(ws, `${colLetra(COL_INICIO + i)}${LINHA_HEADER}`, nome, { bold: true }))

  const cCnpjPart = letraDe("CNPJ Participante"), cRegime = letraDe("Regime IBS/CBS"), cNcm = letraDe("NCM"),
    cVlrItem = letraDe("Vlr Item"), cTipoCredito = letraDe("Tipo Crédito")
  ...
  linhas.forEach((l, i) => {
    ...
    ANOS_CREDITO.forEach(({ anoPremissa }, gi) => {
      ...
      celula(ws, `${cIbsCol}${r}`, f(
        `IF(${cTipoCredito}${r}="Cheio",${cVlrItem}${r}*(${refIbsUF}+${refIbsMUN}),IF(${cTipoCredito}${r}="Alíquota reduzida em 60%",${cVlrItem}${r}*((${refIbsUF}+${refIbsMUN})*0.4),0))`,
        creditoIbs
      ))
