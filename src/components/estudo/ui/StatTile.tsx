...
interface StatTileProps {
  ...
  tone?: Tone;
  className?: string;
  // true = label pode quebrar em 2 linhas em vez de truncar com "..." — usado onde o rótulo é
  // mais longo e a tile fica estreita (ex.: MetaAtualCard, grid de 2 colunas no mobile). Default
  // false preserva o truncamento de sempre nas outras telas (Dashboard/Relatórios).
  wrapLabel?: boolean;
}
...
export default function StatTile({
  icone: Icone, label, valor, sublabel, tone = "primary", className = "", wrapLabel = false,
}: StatTileProps) {
  return (
    <div className={`rounded-xl border border-border bg-card p-4 flex items-center gap-3 ${className}`}>
      ...
      <div className="min-w-0">
        ...
        <p className={`text-xs text-muted-foreground ${wrapLabel ? "leading-snug" : "truncate"}`}>{label}</p>
