...
function CartaVisual({ carta, onExcluir }: { carta: Carta; onExcluir: (id: string) => void }) {
  ...
  return (
    <div className={`relative rounded-2xl border-2 ${cfg.borda} bg-gradient-to-b ${cfg.cor} shadow-xl ${cfg.sombra} p-4 flex flex-col gap-3 group`}>
      ...
      {/* Pergunta */}
      <p className="text-sm text-white/90 leading-snug line-clamp-3 flex-1">
        {carta.frente}
      </p>

      {/* Footer */}
      <div className="flex items-center justify-between">
        <span className="text-[10px] text-white/40 truncate max-w-[55%]">
          {carta.materia ?? "Geral"}
        </span>
...
function SessaoRevisao({
  ...
  return (
    <div className="flex flex-col items-center px-2">
      ...
      {/* Avaliação — só aparece após flip */}
      {flipped && (
...
function FormCriarCarta({
  ...
  const versoPlaceholder =
    tipo === "monstro" ? "Ex: A isenção é a dispensa legal do pagamento do tributo devido, enquanto a imunidade é uma vedação constitucional à própria competência tributária..." :
          ...
          {(["monstro", "armadilha", "tesouro"] as TipoCarta[]).map((t) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-xs font-bold ${sel ? "text-white" : "text-gray-500 dark:text-gray-400"}`}>
                  {cfg.nome}
                </span>
              </button>
            );
...
export default function CartasTab({
  ...
  const [view, setView] = useState<"home" | "criar" | "revisar">("home");
  const [filtroTipo, setFiltroTipo] = useState<TipoCarta | "todas">("todas");

  const hj = hoje();
  const paraHoje = useMemo(() => cartas.filter((c) => c.proximaRevisao <= hj), [cartas, hj]);
  const cartasFiltradas = filtroTipo === "todas" ? cartas : cartas.filter((c) => c.tipo === filtroTipo);
  ...
  if (view === "criar") {
    return (
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 min-h-[500px]">
        <FormCriarCarta onSalvar={handleSalvarCarta} onCancelar={() => setView("home")} />
      </div>
    );
  }
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 min-h-[500px]">
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="ml-auto text-xs bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 px-2 py-0.5 rounded-full font-semibold">
            {paraHoje.length} cartas
          </span>
        </div>
        <SessaoRevisao cartasParaRevisar={paraHoje} onConcluir={handleConcluirRevisao} />
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      {cartas.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          {(["monstro", "armadilha", "tesouro"] as TipoCarta[]).map((t) => {
            const cfg = CARTA_CONFIG[t];
            const Icon = cfg.icone;
            const count = cartas.filter((c) => c.tipo === t).length;
            return (
              <div key={t} className={`rounded-xl border-2 ${cfg.borda} bg-gradient-to-b ${cfg.cor} p-3 flex items-center gap-3 shadow-lg ${cfg.sombra}`}>
                <div className={`w-9 h-9 rounded-lg ${cfg.badge} flex items-center justify-center flex-shrink-0`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2 flex-wrap">
          {(["todas", "monstro", "armadilha", "tesouro"] as const).map((t) => {
            const cfg = t !== "todas" ? CARTA_CONFIG[t] : null;
            const Icon = cfg?.icone;
            return (
              <button
                key={t}
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                  filtroTipo === t
                    ? "bg-[#007cca] text-white shadow-sm"
                    : "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700"
                }`}
              >
                {Icon && <Icon className="h-3 w-3" />}
                {t === "todas" ? "Todas" : CARTA_CONFIG[t].nome}
                <span className="opacity-60">
                  ({t === "todas" ? cartas.length : cartas.filter((c) => c.tipo === t).length})
                </span>
              </button>
            );
          })}
      ...
      {cartasFiltradas.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-gray-400 dark:text-gray-500 text-sm font-medium mb-1">
            {cartas.length === 0 ? "Nenhuma carta criada ainda." : "Nenhuma carta deste tipo."}
          </p>
          <p className="text-gray-300 dark:text-gray-600 text-xs mb-5">
            {cartas.length === 0 && "Crie cartas para começar a revisão espaçada inteligente."}
          </p>
          {cartas.length === 0 && (
            <button
              onClick={() => setView("criar")}
              className="flex items-center gap-2 bg-[#007cca] text-white px-5 py-2.5 rounded-xl text-sm font-semibold shadow-md"
            >
